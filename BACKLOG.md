...
### Ожидают запроса индексирования в GSC

Суточная квота инструмента «Проверка URL» — около 10–12 адресов на ресурс.
Эти страницы уже в `main`, но индексирование по ним ещё не запрашивалось:

- [ ] `https://eventgearpdx.com/blog/av-setup-for-events` (пункт 6, 23.09.2026)
...
7. **GSC — после слияния в `main` и деплоя сайта** (делает владелец в интерфейсе
   ...
   Если суточная квота «Проверки URL» исчерпана, адреса переносятся в список
   «Ожидают запроса индексирования» в начале раздела, а не теряются.
